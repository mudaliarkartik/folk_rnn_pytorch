...
#config 
one_hot = True
embedding_size = 137  # is ignored if one_hot=True
num_layers = 3
rnn_size = 512
dropout = 0.2
...
class FolkRNN(nn.Module):
    ...
    def forward(self, inputs=None, targets=None, sampling_probability=0., batch_size=1):
        ...
        for i in range(1, length):
            ...
            data = data.tolist()
            result = beam_search_decoder(data,137)

            t_out_1 = torch.tensor(result[1][0], dtype= torch.float64).to(self.device)
...
# For sampling
# python pytorch_folk_rnn_train_finetune_sample.py --mode=sample --n_samples=1000 --output_file=rmsprop_samples_bvocab_100 --checkpoint_dir=RMSProp_all_data_train