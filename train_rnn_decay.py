...
class FolkRNN(nn.Module):
    ...
    def forward(self, inputs=None, targets=None, sampling_probability=0., batch_size=1):
        ...
        if inputs is not None:
            ...
            input_embeddings = self.embedding_layer(inputs.to(self.device))
        ...
        for i in range(1, length):
            ...
            if i == 1:
                out = torch.full(size=(1,batch_size), fill_value=81,dtype=torch.int64,device=self.device)
                outputs.append(out)
            else:
...
def sample(model, n_samples, batch_size=64):
    ...
    start_input = torch.full(size=(1,batch_size), fill_value=33, dtype=torch.int64)
    seed_tensor = torch.full(size=(1,batch_size), fill_value=81, dtype=torch.int64)
    inputs = torch.cat((start_input,seed_tensor),0)

    for i in range(n_batches):
        #batch = model.forward(batch_size=batch_size)[0].detach().cpu().numpy().T
        batch = model.forward(inputs=inputs,batch_size=batch_size)[0].detach().cpu().numpy().T
...
# How to use
# For training:
# python train_rnn.py --mode=train --checkpoint_dir=folkrnn_ONeills_datav2_100_lr0.001_decay --sampling_probability=0.0

# For sampling
# python train_rnn_decay.py --mode=sample --n_samples=1000 --output_file=lr0.003_samples --checkpoint_dir=folkrnn_ONeills_datav2_100_lr0.003_O