...
data_path = "data_v2_withtitles_ONeillsJigs"
#data_path = "ONeillsJigs_parsed"
...
'''
#Use this part only when finetuning with ONeills 365 double jigs
    ...
    for row in data:
        ...
        for i in range(len(sequences)):
            ...
            for j in range(len(row)):
                candidate = [seq + [row[j]], score * -log(row[j])]
...
class FolkRNN(nn.Module):
    ...
    def _sample(self, rnn_output, temperature=1.0):
        sampler = torch.distributions.Categorical(probs=rnn_output)
    ...
    def forward(self, inputs=None, targets=None, sampling_probability=0., batch_size=1):
        ...
        for i in range(1, length):
            ...
            prob = torch.nn.functional.softmax(lstm_out, dim=2, dtype= torch.float64)
            data = prob.detach().cpu().clone().numpy()
            data = np.reshape(data,(64,137))
            data = data.tolist()
            result = beam_search_decoder(data,2)

            t_out_1 = torch.tensor(result[0][0], dtype= torch.float64).to(self.device)
            #t_out_2 = torch.tensor(result[1][0], dtype= torch.float64).to(self.device)

            #t_out = torch.mul(t_out_1,t_out_2)

            prob_1 = prob * t_out_1[:,None]

            #t_out = torch.cat((t_out_1,t_out_2),0)
            #t_out = t_out.view(1,64)

            logits.append(lstm_out)
            out = self._sample(prob_1)
            outputs.append(out)
...
def train(model, epochs=20, sampling_probability=0., tb_writer=None):
    ...
    for e in range(epochs):
        if e > learning_rate_decay_after:
            scheduler.step()
        print(f"Epoch: {e}", 'LR:', scheduler.get_lr())
...
def sample(model, n_samples, batch_size=64):
    ...
    for i in range(n_batches):
        ...
        for j in range(batch_size):
            ...
            #Comment the if section you need transcriptions with all time signatures
            #if (txt_sample[:5] == 'M:6/8'):
...
scheduler = StepLR(optimizer, step_size=1, gamma=learning_rate_decay)

count_parameters(model)

if os.path.exists(checkpoint_dir):
    load_checkpoint(checkpoint_dir, model, optimizer)
    #Comment the below two lines if not finetuning 
    optimizer = optim.Adam(model.parameters(), lr = learning_rate)
    scheduler = StepLR(optimizer, step_size=1, gamma=learning_rate_decay)
else:
...
if mode == 'sample':
    ...
    train(model, epochs=100, sampling_probability=sampling_probability, tb_writer=writer)
    
# How to use
# For training:
# python train_rnn_finetune.py --mode=train --checkpoint_dir=folkrnn_ONeills_datav2_100_lr0.003_finetune --sampling_probability=0.0

# For sampling
# python train_rnn_finetune.py --mode=sample --n_samples=1000 --output_file=mysamples_0 --checkpoint_dir=folkrnn_ONeills_datav2_100_lr0.003_finetune